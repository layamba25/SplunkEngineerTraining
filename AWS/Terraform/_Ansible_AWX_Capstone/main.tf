...
resource "aws_security_group" "ansible_security_group" {
  name        = "ansible-sg-capstone"
  description = "Ansible Security Group for Capstone"
...
resource "aws_instance" "ansible" {
  ...
  tags = {
    Name = "Ansible Capstone"
  }
  # Define the user data script to install Ansible
  user_data = <<-EOF
            #!/bin/bash
            echo "Setting hostname to: ${var.aws_hostname}.${var.domain}"
            hostnamectl set-hostname ${var.aws_hostname}.${var.domain}
            apt-get update -y
            apt-get upgrade -y
            ...
            tailscale up --authkey="${var.tailscale_authKey}" --ssh --reset --advertise-tags="${var.tailscale_tags}"
